...
Nel corso degli anni, NuGet ha aggiunto molti servizi che risiedono nello `NuGet.VisualStudio` spazio dei nomi nell' `NuGet.VisualStudio.dll` assembly:

A partire da NuGet 3.3 +, NuGet Esporta quanto segue

- [`IRegistryKey`](#iregistrykey-interface): Metodo per recuperare un valore da una sottochiave del registro di sistema. (3.3 +)
- [`IVsCredentialProvider`](#ivscredentialprovider-interface) Contiene i metodi per ottenere le credenziali per le operazioni NuGet. (4.0 +)
- [`IVsFrameworkCompatibility`](#ivsframeworkcompatibility-interface) Contiene i metodi per individuare i Framework e la compatibilità tra i Framework. (4.0 +)
- [`IVsFrameworkCompatibility2`](#ivsframeworkcompatibility2-interface) Contiene i metodi per individuare i Framework e la compatibilità tra i Framework. (4.0 +)
- [`IVsFrameworkCompatibility3`](#ivsframeworkcompatibility3-interface) Contiene i metodi per individuare i Framework e la compatibilità tra i Framework. (5.8 +)
- [`IVsFrameworkParser`](#ivsframeworkparser-interface) Interfaccia per la gestione della conversione tra stringhe e [FrameworkName](/dotnet/api/system.runtime.versioning.frameworkname) (4.0 +)
- [`IVsFrameworkParser2`](#ivsframeworkparser2-interface) Interfaccia per analizzare le stringhe .NET Framework. Vedere [NuGet-IVsFrameworkParser](https://aka.ms/NuGet-IVsFrameworkParser). (5.8 +)
- [`IVsGlobalPackagesInitScriptExecutor`](#ivsglobalpackagesinitscriptexecutor-interface) Esegui script di PowerShell dai pacchetti in una soluzione (4.0 +)
- [`IVsNuGetFramework`](#ivsnugetframework-interface) Tipo che rappresenta i componenti di un moniker del Framework di destinazione .NET. (5.8 +)
- [`IVsPackageInstaller`](#ivspackageinstaller-interface): Metodi per installare i pacchetti NuGet nei progetti. (3.3 +)
- [' IVsPackageInstaller2](#ivspackageinstaller2-interface) Contiene il metodo per installare la versione più recente di un singolo pacchetto in un progetto all'interno della soluzione corrente.
- [`IVsPackageInstallerEvents`](#ivspackageinstallerevents-interface): Eventi per l'installazione o la disinstallazione di pacchetti. (3.3 +)
- [`IVsPackageInstallerProjectEvents`](#ivspackageinstallerprojectevents-interface): Eventi batch per l'installazione/disinstallazione di pacchetti. (3.3 +)
- [`IVsPackageInstallerServices`](#ivspackageinstallerservices-interface): Metodi per recuperare i pacchetti installati nella soluzione corrente e per verificare se un pacchetto specificato è installato in un progetto. (3.3 +)
- [`IVsPackageManagerProvider`](#ivspackagemanagerprovider-interface): Metodi per fornire suggerimenti di gestione pacchetti alternativi per un pacchetto NuGet. (3.3 +)
- [`IVsPackageMetadata`](#ivspackagemetadata-interface): Metodi per recuperare informazioni su un pacchetto installato. (3.3 +)
- [`IVsPackageProjectMetadata`](#ivspackageprojectmetadata-interface): Metodi per recuperare informazioni su un progetto in cui vengono eseguite le azioni NuGet. (3.3 +)
- [`IVsPackageRestorer`](#ivspackagerestorer-interface): Metodi per ripristinare i pacchetti installati in un progetto. (3.3 +)
- [`IVsPackageSourceProvider`](#ivspackagesourceprovider-interface): Metodi per recuperare un elenco di origini dei pacchetti NuGet. (3.3 +)
- [`IVsPackageUninstaller`](#ivspackageuninstaller-interface): Metodi per disinstallare i pacchetti NuGet dai progetti. (3.3 +)
- [`IVsPathContext`](#ivspathcontext-interface) Informazioni sul percorso NuGet specifiche per il contesto corrente, ad esempio il contesto del progetto. (4.0 +)
- [`IVsPathContext2`](#ivspathcontext2-interface) Informazioni sul percorso NuGet specifiche per il contesto corrente, ad esempio il contesto del progetto. (5.0 +)
- [`IVsPathContextProvider`](#ivspathcontextprovider-interface) Factory per inizializzare le istanze [IVsPathContext](#ivspathcontext-interface) . (4.0 +)
- [`IVsPathContextProvider2`](#ivspathcontextprovider2-interface) Factory per inizializzare le istanze [IVsPathContext2](#ivspathcontext2-interface) . (5.0 +)
- [`IVsProjectJsonToPackageReferenceMigrateResult`](#ivsprojectjsontopackagereferencemigrateresult-interface)  Contiene il risultato dell'operazione di migrazione su un project.jslegacy nel progetto (4.3 +)
- [`IVsProjectJsonToPackageReferenceMigrator`](#ivsprojectjsontopackagereferencemigrator-interface) Contiene i metodi per eseguire la migrazione di un project.jsin un progetto legacy basato su un progetto basato su PackageReference. (4.3 +)
- [`IVsSemanticVersionComparer`](#ivssemanticversioncomparer-interface) Interfaccia per il confronto di due stringhe di versione opache trattate come semantica NuGet (4.0 +)
- [`IVsTemplateWizard`](#ivstemplatewizard-interface): Progettato per i modelli di progetto/elemento per includere i pacchetti preinstallati; Questa interfaccia *non* è pensata per essere richiamata dal codice e non ha metodi pubblici. (3.3 +)

## <a name="using-nuget-services"></a>Uso dei servizi NuGet

1. Installare il [`NuGet.VisualStudio`](https://www.nuget.org/packages/NuGet.VisualStudio) pacchetto nel progetto che contiene l' `NuGet.VisualStudio.dll` assembly.
...
1. Per usare un servizio, importarlo tramite l'[attributo Import di MEF](/dotnet/framework/mef/index#imports-and-exports-with-attributes) o tramite il [servizio IComponentModel](/dotnet/api/microsoft.visualstudio.componentmodelhost.icomponentmodel).
...
## <a name="ivscredentialprovider-interface"></a>Interfaccia IVsCredentialProvider
...
## <a name="ivsframeworkcompatibility-interface"></a>Interfaccia IVsFrameworkCompatibility
...
## <a name="ivsframeworkcompatibility2-interface"></a>Interfaccia IVsFrameworkCompatibility2
...
## <a name="ivsframeworkcompatibility3-interface"></a>Interfaccia IVsFrameworkCompatibility3
...
## <a name="ivsframeworkparser-interface"></a>Interfaccia IVsFrameworkParser
...
## <a name="ivsframeworkparser2-interface"></a>Interfaccia IVsFrameworkParser2
...
## <a name="ivsglobalpackagesinitscriptexecutor-interface"></a>Interfaccia IVsGlobalPackagesInitScriptExecutor
...
## <a name="ivsnugetframework-interface"></a>Interfaccia IVsNuGetFramework
...
## <a name="ivspackageinstaller2-interface"></a>Interfaccia IVsPackageinstaller2
...
## <a name="ivspathcontext-interface"></a>Interfaccia IVsPathContext
...
## <a name="ivspathcontext2-interface"></a>Interfaccia IVsPathContext2
...
## <a name="ivspathcontextprovider-interface"></a>Interfaccia IVsPathContextProvider
...
## <a name="ivspathcontextprovider2-interface"></a>Interfaccia IVsPathContextProvider2
...
## <a name="ivsprojectjsontopackagereferencemigrateresult-interface"></a>Interfaccia IVsProjectJsonToPackageReferenceMigrateResult
...
## <a name="ivsprojectjsontopackagereferencemigrator-interface"></a>Interfaccia IVsProjectJsonToPackageReferenceMigrator
...
## <a name="ivssemanticversioncomparer-interface"></a>Interfaccia IVsSemanticVersionComparer
