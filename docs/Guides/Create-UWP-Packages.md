...
In questa procedura dettagliata si creerà un pacchetto NuGet con un componente UWP nativo (incluso un controllo XAML) che può essere usato in progetti sia gestiti che nativi.

1. [Prerequisiti](#pre-requisites)
1. [Creare un componente Windows Runtime UWP](#create-a-uwp-windows-runtime-component)
...
1. Visual Studio 2017 o Visual Studio 2015. Installare l'edizione Community gratuitamente da [visualstudio.com](https://www.visualstudio.com/). È anche possibile usare le edizioni Professional ed Enterprise.
...
1. Nella finestra di dialogo Compilazione batch fare clic su **Compila** per verificare il progetto e creare i file di output che saranno necessari per il pacchetto NuGet.

> [!Note]
> In questa procedura dettagliata si useranno gli elementi Debug per il pacchetto. Per il pacchetto non di debug, selezionare invece le opzioni Versione nella finestra di dialogo Compilazione batch e fare riferimento alle cartelle Versione risultanti nei passaggi che seguono.
...
Verrà generato `ImageEnhancer.YOUR_NAME.1.0.0.nupkg`. Aprendo il file in uno strumento come [NuGet Package Explorer](https://github.com/NuGetPackageExplorer/NuGetPackageExplorer) ed espandendo tutti i nodi, verranno visualizzati i contenuti seguenti:
...
- [Informazioni di riferimento sul file .nuspec](../schema/nuspec.md)
