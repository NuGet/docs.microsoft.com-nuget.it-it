...
| Elemento | Descrizione |
| --- | --- |
**packageTypes** | *(3.3 +)* Raccolta di zero o più elementi `<packageType>` che specificano il tipo del pacchetto se diverso da un pacchetto di dipendenza tradizionale. Ogni packageType include gli attributi di *name* e *version*. Vedere [Setting a package type](../create-packages/creating-a-package.md#setting-a-package-type) (Impostazione di un tipo di pacchetto). |
...
| --- | --- | 
| `id` | (Obbligatorio) ID del pacchetto di dipendenza. |
...
L'elemento `<group>` senza un attributo `targetFramework` viene usato come elenco predefinito o di fallback delle dipendenze. Vedere [Framework di destinazione](../schema/target-frameworks.md) per gli identificatori di framework esatti.
...
L'elemento `<group>` senza un attributo `targetFramework` viene usato come elenco predefinito o di fallback dei riferimenti. Vedere [Framework di destinazione](../schema/target-frameworks.md) per gli identificatori di framework esatti.
...
| **targetFramework** | (Facoltativo) Specifica il framework di destinazione a cui si applica questo riferimento. Se omesso, indica che il riferimento si applica a tutti i framework. Vedere [Framework di destinazione](../schema/target-frameworks.md) per gli identificatori di framework esatti. |
...
*Versione 3.3+ con project.json e 4.0+ con PackageReference*
...
| **flatten** | Valore booleano che indica se copiare gli elementi di contenuto di una singola cartella nell'output di compilazione (true) o se mantenere la struttura di cartelle nel pacchetto (false). Il valore predefinito è false. |
...
- `TxM` è qualsiasi moniker di framework di destinazione valido supportato da NuGet (vedere [Framework di destinazione](../schema/target-frameworks.md)).
