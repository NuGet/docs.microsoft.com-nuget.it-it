...
*Per i progetti .NET Core che usano NuGet 4.0+, vedere [Pack e restore di NuGet come destinazioni MSBuild](../schema/msbuild-targets.md) per informazioni dettagliate sul crosstargeting.*
...
- [Struttura di cartelle della versione del framework](#framework-version-folder-structure)
- [Corrispondenza tra le versioni di assembly e il framework di destinazione in un progetto](#matching-assembly-versions-and-the-target-framework-in-a-project)
- [Raggruppamento di assembly per versione del framework](#grouping-assemblies-by-framework-version)
- [Determinazione della destinazione NuGet da usare](#determining-which-nuget-target-to-use)
- [File di contenuto e script PowerShell](#content-files-and-powershell-scripts)
...
Per un elenco completo dei nomi supportati, vedere le [informazioni di riferimento sui framework di destinazione](../schema/target-frameworks.md#supported-frameworks).
...
> [!Warning]
> I file di contenuto modificabili e l'esecuzione degli script sono disponibili solo con il formato `packages.config`; sono deprecati quando si usano i formati `project.json` e PackagesReference e non dovrebbero essere usati per i nuovi pacchetti.
