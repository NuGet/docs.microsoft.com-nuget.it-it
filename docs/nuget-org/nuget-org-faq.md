...
**Posso modificare i metadati del pacchetto dopo che sono stati caricati?**
...
In generale, non è supportata l'eliminazione di un pacchetto pubblicato per NuGet.org. Per saperne di più sulla nostra [politica sull'eliminazione dei pacchetti](policies/deleting-packages.md).

**È possibile riservare i nomi per pacchetti che verranno pubblicati in futuro?**

Sì. Puoi riservare gli ID per i pacchetti in [NuGet.org](https://www.nuget.org/) richiedendo un prefisso ID pacchetto per il tuo account. Per richiedere un prefisso per l'ID dei pacchetti, seguire le istruzioni nella [documentazione](id-prefix-reservation.md).

**Come è possibile ottenere l'attestazione di proprietà per i pacchetti?**

Consultate [Gestione dei proprietari dei pacchetti su NuGet.org.](../nuget-org/publish-a-package.md#managing-package-owners-on-nugetorg)
...
**Si consiglia di caricare i pacchetti di test in NuGet.org?**
...
**Qual è la dimensione massima dei pacchetti che posso caricare su NuGet.org?**
...
- Scarica [WinMTR](https://sourceforge.net/projects/winmtr/files/WinMTR-v092.zip/download).
...
Si noti che [l'accesso NuGet.org Password è stato interrotto](https://blog.nuget.org/20180515/NuGet.org-will-only-support-MSA-AAD-starting-June.html) e l'unico modo per accedere a NuGet.org è con un account Microsoft personale (MSA) o Azure Active Directory (AAD). Se tuttavia non si è in grado di accedere agli account AAD o MSA associati, può essere necessario usare l'accesso tramite password per recuperare l'account di NuGet.org. In questa situazione, seguire questa procedura.
- **Requisito:** È necessario avere accesso all'e-mail associata all'account per il quale è necessario recuperare la password.
- Andare alla [pagina Forgot password](https://www.nuget.org/account/ForgotPassword) (Password dimenticata)
- Inserisci l'indirizzo **email** associato all'account NuGet.org che desideri recuperare.
...
- Per effettuare l'accesso con nome utente/password, utilizzare il collegamento **Accedi utilizzando Nuget.org'account** nella [pagina di accesso NuGet.org](https://www.nuget.org/users/account/LogOn).
...
1. Vai alla pagina di [accesso NuGet.org](https://www.nuget.org/users/account/LogOn) e fai clic sul link Hai bisogno di assistenza **per l'accesso.**
...
1. Verrà effettuato il reindirizzamento alla pagina di accesso Microsoft. Si prega di accedere con l'account che si `account2@outlook.com`desidera modificare l'associazione a i.e. **Nota**: potrebbe essere necessario fare clic su **Esci e accedere con un account diverso** durante il flusso di accesso per poter accedere con un account Microsoft diverso.
1. Se viene visualizzato un errore simile al seguente, vedi [L'account Microsoft è collegato a un altro account NuGet.org](#microsoft-account-is-linked-with-another-nugetorg-account) per ulteriori dettagli.
    >_Impossibile aggiornare l'account <account2@outlook.com>Microsoft con 'account2'. Questo problema può verificarsi se è già collegato a un altro account NuGet.This could happen if it is already linked to another NuGet account. Per ulteriori informazioni, contattare il supporto tecnico._
...
**Ho dimenticato di avere un altro account NuGet.org, come faccio a scoprire quale NuGet.org conto è?**

Accedere con il secondo account Microsoft nella [pagina di accesso](https://www.nuget.org/users/account/LogOn?returnUrl=%2F# "pagina di accesso"). In questo modo si accederà all'account di NuGet.org attualmente associato al secondo account Microsoft. Sarà quindi possibile visualizzare i pacchetti caricati ed eseguire operazioni di gestione di questo account.

**Non mi interessa questo secondo account NuGet.org, voglio cambiare il mio login per il primo account NuGet.org con il secondo account Microsoft. Cosa devo fare?**
...
1. Seguire i passaggi per eliminare l'utente per il secondo account [delete user](#how-to-delete-my-nugetorg-account) `MyNuGetAccount2`NuGet.org . 
...
**Aspetta, mi interessa anche questo secondo conto. Non voglio perdere questo account, ma modificare il mio account associato login per il primo account.**
...
1. Per prima cosa devi effettuare `account2@outlook.com` il login con il tuo secondo account Microsoft, su NuGet.org. Seguire i passaggi precedenti per modificare gli account di accesso associati e associare il terzo account Microsoft a questo account NuGet.org.
...
**Come faccio a trovare quale account Microsoft è collegato a questo account NuGet.org?**

È consigliabile utilizzare il flusso di assistenza per l'accesso per individuare `account1@outlook.com`l'account Microsoft collegato all'account NuGet.org con l'indirizzo di posta elettronica . [sign in assistance](#which-microsoft-account-is-linked-to-my-nugetorg-account)

**Si vuole sostituire tale account con un account Microsoft**

Seguire la procedura descritta in [Impossibile utilizzare microsoft login, come è possibile recuperare la sezione dell'account NuGet.org](#unable-to-use-microsoft-login-how-do-i-recover-my-nugetorg-account) per associare l'account Microsoft all'account NuGet.org esistente.
...
Se si è tentato di utilizzare [l'assistenza](#which-microsoft-account-is-linked-to-my-nugetorg-account) per l'accesso e non si ha accesso all'account Microsoft associato all'account NuGet.org, attenersi alla procedura seguente per collegare un nuovo account Microsoft all'account NuGet.org.
1. **Requisito**: È necessario accedere a un account Microsoft non associato ad alcun account NuGet.org esistente. Se non se ne ha alcuno, è possibile [crearne uno](https://signup.live.com).
...
3. [Accedi per NuGet.org](https://www.nuget.org/users/account/LogOnNuGetAccount) utilizzando il nome utente/password login.
...
5. **NOTA**: Ignorare l'istruzione di accesso con l'account Microsoft specificato. È ora possibile collegare l'account di NuGet.org a qualsiasi altro account di accesso Microsoft.
...
1. **Requisito:** è necessario che un singolo account venga creato per la prima volta su NuGet.org per essere utilizzato come amministratore nell'account dell'organizzazione. Se non ne hai uno, [crea un nuovo account NuGet.org](individual-accounts.md)
2. Seguire la [procedura per recuperare l'accesso della password](#how-to-recover-nugetorg-password-login) per l'account NuGet.org se non si dispone di password di accesso per esso, se lo fai, saltare questo passaggio.
3. [Accedi per NuGet.org](https://www.nuget.org/users/account/LogOnNuGetAccount) utilizzando il nome utente/password login.
4. Dopo l'accesso, verrà visualizzata la finestra di dialogo popup illustrata più avanti. Si tratta di finestra di dialogo che informa della sospensione delle password. 
    > [!Important]
...
**Che cos'è questo stato non gestito durante l'accesso? E perché sta succedendo adesso?** 
...
È possibile [creare](https://www.microsoft.com/account) un nuovo account Microsoft (con un indirizzo e-mail **non** associato a @yourdomain.com). Segui i passaggi indicati nella sezione [Recuperare il tuo account NuGet.org.](#unable-to-use-microsoft-login-how-do-i-recover-my-nugetorg-account)

### <a name="how-do-i-change-my-nugetorg-account-username"></a>Come si cambia il nome utente dell'account di NuGet.org?

Non è possibile. Per una questione di politica non permettiamo la modifica dei nomi utente. Inoltre, questa operazione è una modifica sostanziale per gli utenti che potrebbero aver definito criteri di [attendibilità del pacchetto in base al proprietario del pacchetto.](../consume-packages/installing-signed-packages.md#trust-package-owners) L'unico modo per cambiare il nome utente consiste nel creare un nuovo account con il nome utente desiderato. È consigliabile eliminare l'account esistente prima di crearne uno nuovo. In caso contrario non sarà possibile riutilizzare l'account Microsoft registrato.
> [!Important]
...
Segui i passaggi indicati nella sezione [Eliminare il tuo account NuGet.org](#how-to-delete-my-nugetorg-account) e [registrare un nuovo account](individual-accounts.md) con il nome utente corretto.
...
1. [Accedi a NuGet.org](https://www.nuget.org/users/account/LogOn) con l'account che desideri eliminare.
2. Clicca su questo [https://www.nuget.org/account/delete](https://www.nuget.org/account/delete) url: e segui i passaggi per inviare la richiesta di eliminazione dell'account.
