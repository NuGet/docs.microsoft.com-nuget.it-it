...
Sì. È possibile riservare gli ID per i pacchetti in [NuGet.org](https://www.nuget.org/) richiedendo un prefisso per l'ID pacchetto per l'account. Per richiedere un prefisso per l'ID dei pacchetti, seguire le istruzioni nella [documentazione](id-prefix-reservation.md).
...
Si noti che l'account di [accesso con password di NuGet.org è stato interrotto](https://blog.nuget.org/20180515/NuGet.org-will-only-support-MSA-AAD-starting-June.html) e l'unico modo per accedere a NuGet.org è costituito da un account Personal account Microsoft (MSA) o Azure Active Directory (AAD). Se tuttavia non si è in grado di accedere agli account AAD o MSA associati, può essere necessario usare l'accesso tramite password per recuperare l'account di NuGet.org. In questa situazione, seguire questa procedura.
...
- Immettere l'indirizzo di **posta elettronica** associato all'account NuGet.org che si desidera ripristinare.
- Fare clic sul pulsante **Send** (Invia).
...
- Per eseguire l'accesso con nome utente/password, usare il collegamento Accedi con l' **account NuGet.org** nella  [pagina di accesso di NuGet.org](https://www.nuget.org/users/account/LogOn).
...
1. Passare alla [pagina di accesso a NuGet.org](https://www.nuget.org/users/account/LogOn) e fare clic su **richiesta di assistenza** per l'accesso? collegamento.
...
**Ho dimenticato di avere un altro account NuGet.org, come è possibile individuare l'account NuGet.org?**
...
1. Seguire i passaggi per [eliminare l'utente](#how-to-delete-my-nugetorg-account) per il secondo account NuGet.org `MyNuGetAccount2` . 
...
1. Prima di tutto è necessario effettuare l'accesso con la seconda account Microsoft, `account2@outlook.com` su NuGet.org. Attenersi alla procedura descritta in precedenza per modificare gli account di accesso associati e associare la terza account Microsoft a questo account NuGet.org.
...
**Ricerca per categorie trovare la account Microsoft collegata a questo account NuGet.org?**

È consigliabile usare il flusso di assistenza per l' [accesso](#which-microsoft-account-is-linked-to-my-nugetorg-account) per determinare quali account Microsoft sono collegati all'account NuGet.org con l'indirizzo di posta elettronica `account1@outlook.com` .

**Si vuole sostituire tale account con un account Microsoft**

Seguire la procedura descritta in non è possibile usare l'account di [accesso Microsoft. come si ripristina la sezione account NuGet.org](#unable-to-use-microsoft-login-how-do-i-recover-my-nugetorg-account) per associare l'account Microsoft all'account NuGet.org esistente.
...
Questo può essere accaduto se l'utente corrente o un altro utente dell'organizzazione (con indirizzo di posta elettronica @yourdomain.com) si è registrato con uno dei servizi integrati di AAD o ha effettuato un'[iscrizione self-service per Azure Active Directory](/azure/active-directory/users-groups-roles/directory-self-service-signup), che crea un tenant "non gestito"per il dominio dell'account Microsoft usato (@yourdomain.com in questo caso). 
...
Se vuole accedere a NuGet.org con l'account Microsoft (@yourdomain.com), l'utente (o un amministratore della società) dovrà attestare la proprietà dell'account tenant AAD tramite una convalida DNS per l'autenticazione degli utenti con indirizzo di posta elettronica "@yourdomain.com". Seguire la procedura per l'[acquisizione come amministratore di domini](/azure/active-directory/users-groups-roles/domains-admin-takeover) documentata da Azure Active Directory. Dopo l'esecuzione della procedura, l'accesso normale inizierà a funzionare.

**Se non si vogliono eseguire tutte queste operazioni, qual è l'altro modo per recuperare l'account?**

È possibile [creare](https://www.microsoft.com/account) un nuovo account Microsoft (con un indirizzo e-mail **non** associato a @yourdomain.com). Seguire i passaggi indicati nella sezione [ripristinare l'account NuGet.org](#unable-to-use-microsoft-login-how-do-i-recover-my-nugetorg-account) .
...
1. [Accedere a NuGet.org](https://www.nuget.org/users/account/LogOn) con l'account che si vuole eliminare.
2. Fare clic su questo URL: [https://www.nuget.org/account/delete](https://www.nuget.org/account/delete) e seguire i passaggi per inviare la richiesta di eliminazione dell'account.
