...
ms.locfileid: "76813273"
---
# <a name="package-metadata"></a>Metadati del pacchetto

È possibile recuperare i metadati relativi ai pacchetti disponibili in un'origine del pacchetto tramite l'API NuGet V3. I metadati possono essere recuperati usando la risorsa `RegistrationsBaseUrl` trovata nell' [indice del servizio](service-index.md).

La raccolta dei documenti disponibili in `RegistrationsBaseUrl` sono spesso denominate "registrazioni" o "BLOB di registrazione". Il set di documenti in un singolo `RegistrationsBaseUrl` viene definito "hive di registrazione". Un hive di registrazione contiene tutti i metadati relativi a ogni pacchetto disponibile in un'origine del pacchetto.
...
Queste registrazioni non vengono compresse (ovvero usano un `Content-Encoding: identity`implicito). I pacchetti SemVer 2.0.0 sono **esclusi** da questo hive.
...
L'URL di base per le API seguenti è il valore della proprietà `@id` associata ai valori di `@type` di risorse sopra indicati. Nel documento seguente verrà usato l'URL di base segnaposto `{@id}`.
...
L'URL dell'indice di registrazione è prevedibile e può essere determinato dal client in base all'ID del pacchetto e al valore `@id` della risorsa di registrazione dall'indice del servizio. Gli URL per le pagine di registrazione e le foglie vengono individuati controllando l'indice di registrazione.
...
Name     | In     | Tipo di    | Richiesto | Note
-------- | ------ | ------- | -------- | -----
LOWER_ID | URL    | string  | sì      | ID del pacchetto, minuscolo

Il valore `LOWER_ID` è l'ID pacchetto desiderato in minuscolo usando le regole implementate da. Metodo di [`System.String.ToLowerInvariant()`](/dotnet/api/system.string.tolowerinvariant?view=netstandard-2.0#System_String_ToLowerInvariant) di NET.
...
Ogni elemento nella matrice di `items` dell'oggetto index è un oggetto JSON che rappresenta una pagina di registrazione.
...
I limiti `lower` e `upper` dell'oggetto pagina sono utili quando sono necessari i metadati per una specifica versione di pagina.
...
La proprietà `parent` verrà visualizzata solo se l'oggetto pagina di registrazione dispone della proprietà `items`.

Se la proprietà `items` non è presente nell'oggetto pagina di registrazione, è necessario utilizzare l'URL specificato nel `@id` per recuperare i metadati relativi alle singole versioni del pacchetto. La matrice `items` viene talvolta esclusa dall'oggetto pagina come ottimizzazione. Se il numero di versioni di un singolo ID pacchetto è molto grande, il documento dell'indice di registrazione sarà massiccio ed inutile da elaborare per un client che si occupa solo di una versione specifica o di una piccola gamma di versioni.

Si noti che se la proprietà `items` è presente, non è necessario utilizzare la proprietà `@id`, poiché tutti i dati della pagina sono già stati inline nella proprietà `items`.

Ogni elemento nella matrice di `items` dell'oggetto pagina è un oggetto JSON che rappresenta un foglio di registrazione ed è associato ai metadati.
...
catalogEntry   | Oggetto di | sì      | Voce di catalogo contenente i metadati del pacchetto
...
La proprietà `catalogEntry` nell'oggetto foglia di registrazione presenta le proprietà seguenti:
...
deprecazione              | Oggetto di                     | No       | La deprecazione associata al pacchetto
Descrizione              | string                     | No       | 
iconUrl                  | string                     | No       | 
ID                       | string                     | sì      | ID del pacchetto
licenseUrl               | string                     | No       |
licenseExpression        | string                     | No       | 
elenco                   | boolean                    | No       | Deve essere considerato come elencato se assente
...
title                    | string                     | No       | 
Versione di                  | string                     | sì      | Stringa di versione completa dopo la normalizzazione

Il pacchetto `version` proprietà è la stringa di versione completa dopo la normalizzazione. Ciò significa che i dati di compilazione di SemVer 2.0.0 possono essere inclusi qui.
...
Il valore della proprietà `licenseExpression` è conforme alla [sintassi delle espressioni di licenza NuGet](../reference/nuspec.md#license).
...
La stringa `targetFramework` usa il formato implementato dalla libreria .NET di NuGet [. Frameworks](https://www.nuget.org/packages/NuGet.Frameworks/)di NuGet. Se non viene specificato alcun `targetFramework`, il gruppo di dipendenze si applica a tutti i Framework di destinazione.

La proprietà `dependencies` è una matrice di oggetti, ognuno dei quali rappresenta una dipendenza del pacchetto corrente.
...
range        | Oggetto di | No       | Intervallo di [versioni](../concepts/package-versioning.md#version-ranges-and-wildcards) consentite della dipendenza
...
alternatePackage | Oggetto di           | No       | Il pacchetto alternativo da usare
...
range        | Oggetto di | No       | Intervallo di [versioni](../concepts/package-versioning.md#version-ranges-and-wildcards)consentito o `*` se è consentita una versione
