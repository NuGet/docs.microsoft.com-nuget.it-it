...
NuGet 4.0 + può usare direttamente le informazioni contenute in un file `.csproj` senza richiedere un file `.nuspec` o `project.json` separato. Tutti i metadati archiviati in precedenza in questi file di configurazione possono essere invece archiviati direttamente nel file `.csproj`, come descritto di seguito.
...
- [Ordine di compilazione delle destinazioni](#target-build-order)
- [Destinazione pack](#pack-target)
- [Scenari pack](#pack-scenarios)
- [Destinazione restore](#restore-target)
...
| VersionPrefix | PackageVersionPrefix | vuoto | L'impostazione di PackageVersion sovrascriverà PackageVersionPrefix |
| VersionSuffix | PackageVersionSuffix | vuoto | $(VersionSuffix) da MSBuild. L'impostazione di PackageVersion sovrascriverà PackageVersionSuffix | 
