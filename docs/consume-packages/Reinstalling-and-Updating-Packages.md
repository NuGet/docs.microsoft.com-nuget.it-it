...
Per impostazione predefinita, `Update-Package` influisce su tutti i progetti in una soluzione. Per limitare l'azione a un progetto specifico, usare l'opzione `-ProjectName`, con il nome del progetto come viene visualizzato in Esplora soluzioni:
...
Per informazioni dettagliate sul comando, vedere le informazioni di riferimento su [Update-Package](../reference/ps-reference/ps-ref-update-package.md).
