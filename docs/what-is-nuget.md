...
- [`packages.config`](reference/packages-config.md): *(NuGet 1.0 +)* file XML che gestisce un elenco semplice di tutte le dipendenze nel progetto, incluse le dipendenze di altri pacchetti installati. I pacchetti installati o ripristinati vengono archiviati in una cartella `packages`.

Il formato di gestione dei pacchetti usato in un determinato progetto dipende dal tipo di progetto e dalla versione di NuGet (e/o Visual Studio) disponibile. Per verificare il formato in uso, è sufficiente cercare `packages.config` nella radice del progetto dopo l'installazione del primo pacchetto. Se non si dispone di tale file, cercare direttamente nel file di progetto un \<PackageReference\> elemento.
