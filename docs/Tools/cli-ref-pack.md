...
Crea un pacchetto NuGet in base all'opzione `.nuspec` o file di progetto. Il `dotnet pack` comando (vedere [comandi dotnet](dotnet-Commands.md)) e `msbuild /t:pack` (vedere [destinazioni di MSBuild](../schema/msbuild-targets.md)) possono essere utilizzati come valori alternativi.
